...
int main(int argc, char** argv) {
    ...
    if (rank < size - 1) {
        MPI_Isend(xlocal[maxn / size], maxn, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, &request);
        MPI_Wait(&request, &status);
    }
    if (rank > 0) {
        MPI_Irecv(xlocal[0], maxn, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, &request);
        MPI_Wait(&request, &status);
    }
    ...
    if (rank < size - 1) {
        MPI_Irecv(xlocal[maxn / size + 1], maxn, MPI_DOUBLE, rank + 1, 1, MPI_COMM_WORLD, &request);
        MPI_Wait(&request, &status);
    }
